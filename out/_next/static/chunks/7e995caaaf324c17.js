...
`;r.ShaderStore.IncludesShadersStoreWGSL[a]||(r.ShaderStore.IncludesShadersStoreWGSL[a]=c),e.s([])},90512,e=>{"use strict";var r=e.i(47662);e.i(64500);let a="iblScaledLuminancePixelShader",c=`#include<helperFunctions>
...
return dot(color,LuminanceEncodeApprox);}
@fragment
fn main(input: FragmentInputs)->FragmentOutputs {var deform: f32=sin(input.vUV.y*PI);var luminance: f32=fetchLuminance(input.vUV);fragmentOutputs.color=vec4f(vec3f(deform*luminance),1.0);}`;r.ShaderStore.ShadersStoreWGSL[a]||(r.ShaderStore.ShadersStoreWGSL[a]=c),e.s(["iblScaledLuminancePixelShaderWGSL",0,{name:a,shader:c}])}]);